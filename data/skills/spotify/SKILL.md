...
If `authorization` is missing, run `scripts/spotify auth start` and send the URL it prints, exactly as printed. There is no argument to pass, the script reads the client ID itself. The URL carries `${SPOTIFY_CLIENT_ID}` where the client ID belongs and that is deliberate, not a bug to fix. `send_message` substitutes the real value as the message goes out. Do not look the value up and do not rewrite the URL, or the owner gets a link that does not work.
