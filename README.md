...
Official release assets are `tera-x86_64-unknown-linux-gnu`, `tera-aarch64-unknown-linux-gnu` and `tera-aarch64-apple-darwin`, each with a matching `.sha256` file. The arm64 Linux build is made on glibc 2.35 so it runs on Raspberry Pi OS Bookworm. Builds made outside Git carry `unknown` as the commit SHA. Set `TERA_GIT_SHA` in the build environment when packaging from an exported source tree.
...
The health skill keeps a local SQLite history under `<workspace>/.runtime/health/`. It syncs Garmin Connect through the unofficial Garmin Connect API that [garminconnect](https://github.com/cyberjunky/python-garminconnect) wraps, including the emailed MFA code sent back over WhatsApp, and reads standard Bluetooth heart rate straps, scales, blood pressure cuffs, thermometers and pulse oximeters. A Garmin watch works over Bluetooth too, while it broadcasts heart rate. Briefs, trends and recommendations compare each day with the owner's own 28 day normal and run on the standard library, so only syncing and Bluetooth need the pinned packages that setup installs into a private virtualenv.
